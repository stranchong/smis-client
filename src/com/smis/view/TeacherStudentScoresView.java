...
public class TeacherStudentScoresView implements OnClickListener, OnScrollListener {
	public interface LoginDelegate {
		public void loadMoreData(int currentCount);
	}

	private Activity ac;
	private ListView lvStudentScores;
	private View listfoot;
	private Button btnFoot;
	private ProgressBar pbFoot;
	// 定义接口对象
	private TeacherStudentScoresView.LoginDelegate dlg;
	// 适配器
	...
	public TeacherStudentScoresView(Activity ac, TeacherStudentScoresView.LoginDelegate delegate) {
		this.ac = ac;
		dlg = delegate;
		// 实例化ListView控件和其底部布局
		lvStudentScores = (ListView) ac.findViewById(R.id.lvStudentScores);
		...
		lvStudentScores.setOnScrollListener(this);
		lvStudentScores.setOnCreateContextMenuListener(ac);
	}
	...
	public void onClick(View v) {
		if (adapter.getCount() < totalCount) { // 当前加载的数据条数小于总条数
			...
			lvStudentScores.removeFooterView(listfoot);
	...
	public void printInitData(List<HashMap<String, String>> initData) {
		data = initData;
		// 实例化adapter
		adapter = new SimpleAdapter(ac, data, R.layout.listitem_teacher_student_score, new String[] { "cname", "clname", "sname",
				"score" }, new int[] { R.id.tvCname, R.id.tvClname, R.id.tvSname, R.id.tvScore });
		// lvCourseScore加上底部布局，注意要放在setAdapter方法前
		lvStudentScores.addFooterView(listfoot);
		// 显示数据
		lvStudentScores.setAdapter(adapter);
	}
	...
	public void updateItemData(int position, Map<String, String> newItemData) {
		...
		// 旧数据改为新数据
		itemData.put("score", newItemData.get("score"));
		// 刷新数据
		adapter.notifyDataSetChanged();
	}
