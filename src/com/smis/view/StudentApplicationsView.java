...
public class StudentApplicationsView implements OnClickListener, OnScrollListener {
	...
	// 定义接口对象
	private StudentApplicationsView.LoginDelegate dlg;
	// 适配器
	...
	public StudentApplicationsView(Activity ac, StudentApplicationsView.LoginDelegate delegate) {
		this.ac = ac;
		dlg = delegate;
		// 实例化ListView控件和其底部布局
	...
	public void printInitData(List<HashMap<String, String>> initData) {
		data = initData;
		// 实例化adapter
		adapter = new StudentApplicationsAdapter(ac, data, R.layout.listitem_student_application_list, new String[] { "aid",
				"createDate", "startDate", "overDate", "countDay", "leaveReason", "leavePlace", "confirmMark", "confirmName",
				"confirmDate", "confirmReply" }, new int[] { R.id.tvAid, R.id.tvCreateDate, R.id.tvStartDate, R.id.tvOverDate,
				R.id.tvCountDay, R.id.tvLeaveReason, R.id.tvLeavePlace, R.id.tvConfirmMark, R.id.tvConfirmName,
				R.id.tvConfirmDate, R.id.tvConfirmReply });
	 ...
	 * @param map
	 */
	public void updateItemData(int position, Bundle map) {
		// 获取旧的数据对象
		HashMap<String, String> itemData = data.get(position);
		// 旧数据改为新数据
		itemData.put("startDate", map.getString("startDate"));
		itemData.put("overDate", map.getString("overDate"));
		itemData.put("leavePlace", map.getString("leavePlace"));
		itemData.put("leaveReason", map.getString("leaveReason"));
		itemData.put("countDay", map.getString("countDay"));
		// 刷新数据
		adapter.notifyDataSetChanged();
		// 弹出修改成功提示框
		Toast.makeText(ac, map.getString(BusinessResponse.MSG), Toast.LENGTH_SHORT).show();
	}
	 ...
	 * @param msg
	 */
	public void deleteItemData(int position, boolean isSuccess, String msg) {
		// 删除成功
		if (isSuccess) {
			// 移除对应的item
